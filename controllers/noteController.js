...
const repositionNote = async ({ refUuid }) => {
  const refNote = refUuid
    ? await Note.findOne({ where: { uuid: refUuid }, isDeleted: false })
    : null;
...
exports.createNote = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: 'Successfully created', note });
  } catch (error) {
...
exports.getSpecificNote = async (req, res, next) => {
  ...
  try {
    ...
    if (!note)
      return res
        .status(422)
        .json({ success: false, message: 'Unable to find note' });
    res.status(201).json(note);
  } catch (error) {
    return res.status(422).json({ success: false, message: 'error: ', error });
  }
...
exports.updateNotePosition = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: 'Successfully update', data: note });
  } catch (error) {
    return res.status(422).json({ success: false, message: 'error: ', error });
  }
};
